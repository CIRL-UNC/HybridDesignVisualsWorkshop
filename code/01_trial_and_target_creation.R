...
if (operating_system == "Windows") {
  ...
  prgdir <- "F:/repo/HybridDesignVisualsWorkshop/code/"
  # Naming the folder where the trial data are being kept
  cohort <- "F:/repo/HybridDesignVisualsWorkshop/data/Trial data/"
  #Naming the folder where the target data are being kept
  targ <- "F:/repo/HybridDesignVisualsWorkshop/data/Target data/"
  #Naming the folder where the figures are output
  figdir <- "F:/repo/HybridDesignVisualsWorkshop/Visualizations/"
}
...
# limit to complete cases
set.seed(122355)
analytictargcohort <- read_sas(paste0(targ, "seerworkshop.sas7bdat")) %>%
  rename_all(.funs = list(tolower)) %>%
  mutate(
    intrial = 0,
    age = final_interval_age_calculated,
    colon = case_when(
      site_recode_icd_o_3_who_2008 %in% c(25, 26) ~ 0, # flag
      TRUE ~ 1
    ),
    female = case_when(
      sex == 2 ~ 1,
    ...
    livermets = case_when(
      seercombinedmetsatdx_liver_2010 >=  1 ~ 1,
      TRUE ~ 0
    ),
    wildkras = rbinom(n(), 1, 0.9)
  ) %>%
